...
OUTPUT_ROOT = Path(
    "data/processed/cbis_ddsm/model_input/full_mammograms_v4"
)
...
MAX_LATERAL_CROP_FRACTION = 0.25
...
def lateral_only_crop(image, mask, laterality):
    """
    Preserve full image height and the complete chest-wall edge.
    Crop only obvious background on the nipple side.
    """
    ...
    lateral_margin = int(
        round(breast_width * SAFETY_MARGIN_FRACTION)
    )

    max_lateral_crop = int(
        round(width * MAX_LATERAL_CROP_FRACTION)
    )

    edge_protection = int(
        round(width * EDGE_PROTECTION_FRACTION)
    )

    laterality = normalise_laterality(laterality)

    # Full vertical dimension is always retained.
    crop_y_min = 0
    crop_y_max = height - 1

    if laterality == "LEFT":
        # Chest wall is protected on the left.
        crop_x_min = 0

        proposed_x_max = min(
            width - 1,
            x_max + lateral_margin
        )

        if (width - 1 - x_max) <= edge_protection:
            crop_x_max = width - 1
        else:
            crop_x_max = max(
                proposed_x_max,
                width - 1 - max_lateral_crop
            )

    elif laterality == "RIGHT":
        # Chest wall is protected on the right.
        crop_x_max = width - 1

        proposed_x_min = max(
            0,
            x_min - lateral_margin
        )

        if x_min <= edge_protection:
            crop_x_min = 0
        else:
            crop_x_min = min(
                proposed_x_min,
...
print("=" * 72)
print("FULL-MAMMOGRAM PREPROCESSING V4")
print("=" * 72)
...
for index, row in tqdm(
    ...
    try:
        ...
        df.at[index, "total_removed_fraction"] = total_removed_fraction
        ...
        df.at[index, "foreground_fraction_before_crop"] = foreground_before
        df.at[index, "foreground_fraction_after_crop"] = foreground_after
...
print("\n")
print("=" * 72)
print("PREPROCESSING V4 SUMMARY")
print("=" * 72)
...
vertical_crop = (
    (pd.to_numeric(successful["crop_top_fraction"]) > 0)
    |
    (pd.to_numeric(successful["crop_bottom_fraction"]) > 0)
)

print("\nImages with any vertical crop:")
print(f"{vertical_crop.sum():,}")

chest_wall_crop = (
    (
        (successful["laterality"] == "LEFT")
        &
        (pd.to_numeric(successful["crop_left_fraction"]) > 0)
    )
    |
    (
        (successful["laterality"] == "RIGHT")
        &
        (pd.to_numeric(successful["crop_right_fraction"]) > 0)
    )
)

print("\nImages where chest-wall edge was cropped:")
print(f"{chest_wall_crop.sum():,}")

print("\nImages with >25% total area removed:")
print(
    (
        pd.to_numeric(
            successful["total_removed_fraction"]
